...
function playChime(ctx: AudioContext, gain: number): void {
...
function playSuccess(ctx: AudioContext, gain: number): void {
  const now = ctx.currentTime
...
function playSubtle(ctx: AudioContext, gain: number): void {
  const now = ctx.currentTime
  const osc = ctx.createOscillator()
  const gainNode = ctx.createGain()
